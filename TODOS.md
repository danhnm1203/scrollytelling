...
| `twitter:url` on the card | Twitter falls back to `og:url`, which every template emits, so the tag would be a second copy of a value that is already there — and a second thing to keep in step. Worth adding only if a consumer turns up that reads one and not the other. Decided while doing #79, which asked for a decision rather than an implementation. |
| The repository's social preview upload | The image is made and committed at `docs/media/social-preview.png` (1280×640, a real page this tool built). GitHub exposes no API for the social preview — the REST response does not carry the field — so it is Settings → General → Social preview → Edit → upload that file, then paste the repository url somewhere that unfurls to confirm. Tracked as #80. Recorded here because an issue comment is easy to lose and this is the only step left. |
