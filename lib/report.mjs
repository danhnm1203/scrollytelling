 ...
 *
 * `fromRow` is where the region starts vertically: the whole frame for copy
 * placed across it, the lower half for copy anchored to the bottom.
 */
function regionOfFrame(grid, region, fromRow = 0) {
  const cols = REGION_COLUMNS[region];
  let total = 0;
  let n = 0;
  for (let row = fromRow; row < LUMA_ROWS; row++) {
...
/**
 * The whole story object, not just its beats.
 *
 * The outline a zero-build page carries needs the brand and the description
 * too, and pulling those out with separate regexes would be a second way of
 * reading the same file that could disagree with the first.
 */
export function parseStory(source) {
  return parseLiteral(source, /\bstory\s*=\s*\{/, "story", "{", "}");
}
...
function parseArrayLiteral(source, pattern, name) {
  return parseLiteral(source, pattern, name, "[", "]");
}

/**
 * Finds a named literal by matching its brackets, and evaluates just that.
 *
 * See parseBeats for why evaluation rather than a regex over the contents.
 */
function parseLiteral(source, pattern, name, openChar, closeChar) {
  const key = pattern.exec(source);
  if (!key) {
    throw new Error(
      `no \`${name}\` found in this file.\n` +
        "  Expected a file generated or scaffolded by scrollytelling.",
    );
  }
  ...
  if (close === -1) throw new Error(`the \`${name}\` literal is not closed`);
  ...
  // The shape check follows the bracket we matched: an array literal must
  // evaluate to an array, an object literal to an object. Getting something
  // else back means the file is not what it claims to be, and reading on would
  // produce confusing failures further downstream.
  const expected = openChar === "[" ? "array" : "object";
  const ok = expected === "array" ? Array.isArray(value) : value && typeof value === "object";
  if (!ok) throw new Error(`\`${name}\` is not an ${expected}`);
  return value;
}
...
export function beatLuma(sequence, beat) {
  ...
  // `anchor` picks the rows and `align` the columns, the same two axes the
  // page places copy on. Coarser than the runtime scrim, which intersects the
  // bands with the part of the frame this viewport actually shows — this is a
  // report about a frame, not about a viewport. Averaging the full width for a
  // bottom-anchored beat, though, reports one number for two beats sitting on
  // opposite sides of the same frame, and where the copy is readable is the
  // whole job of this check.
  const region = beat.align === "left" || beat.align === "right" ? beat.align : "centre";
  const fromRow = beat.anchor === "bottom" ? Math.ceil(LUMA_ROWS / 2) : 0;
  return regionOfFrame(grid, region, fromRow);
}
