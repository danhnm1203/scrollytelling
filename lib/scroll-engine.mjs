...
/**
 * How long the picture takes to resolve onto a whole frame after the scroll
 * stops.
 *
 * Shorter than the scrub, because this is not motion anybody is reading — it is
 * the blur clearing. Long enough not to be a pop.
 */
const SETTLE_SECONDS = 0.16;

/**
 * How long the copy takes to resolve onto a single beat after the scroll stops.
 *
 * Slower than the picture, so it reads as the page deciding rather than as a
 * heading being switched off under the reader.
 */
...
function startScrubbing({
  ...
  // Beats are authored against what the footage is doing, so they land
  // mid-frame — the one place a reader stops to read is the one place two
  // frames are composited. Pulled onto whole frames once here, where the frame
  // count is finally known, rather than corrected under them after they stop.
  // The move is at most half a frame, well inside the stretch each beat holds.
  const story = rawStory
    ? { ...rawStory, sections: alignBeats(rawStory.sections ?? [], sequence.totalFrames) }
    : rawStory;

  // One snap anchor per beat, in the runway, at that beat's aligned position.
  // The engine never writes to scrollY: it places resting points and the
  // browser owns the gesture from there, so this cannot fight native scroll,
  // scroll anchoring or the keyboard the way a scroll listener would.
  const anchors = placeAnchors(container, story?.sections ?? [], env);
  ...
  let raf = 0;
  let eased = 0;
  /** The frame position actually painted, in frames. Trails `eased` only once
      the scroll has stopped, while it resolves onto a whole frame. */
  let drawn = 0;
  /** How far the copy has resolved onto one beat. 0 whenever the scrub moves. */
  let focus = 0;
  let lastPaint = 0;
  let primed = false;
  let stopped = false;
  ...
  const draw = (now) => {
    ...
    // Once the scrub has stopped there is no motion left to smooth, and a
    // half-blended pair reads as a double exposure rather than as blur. Resolve
    // onto one frame; while it is still moving this is the scrub position
    // untouched, so the picture never trails the copy.
    const settled = settlePosition({
      drawn,
      exact: frameIndex(stepped.eased, sequence.totalFrames),
      moving: stepped.animating,
    ...
    // The same argument applied to the words: a crossfade is a state to pass
    // through, not one to leave on a page nobody is scrolling.
    const focused = settleFocus({
      focus,
      moving: stepped.animating,
    ...
    // sequence wherever it had eased to — part way onto a frame, or part way
    // through resolving the copy.
    if (stepped.animating || settled.settling || focused.settling) schedule();
    ...
    paintOverlays({ overlays, story, sequence, exact, eased, focus, vw, vh, ready });

    const blend = blendFrames({ exact, held: frames.keys(), totalFrames: sequence.totalFrames });
    if (blend.base === null) return;

    const img = frames.get(blend.base);
    if (!img) return;

    const rect = drawRect({ viewportWidth: vw, viewportHeight: vh, sequence });
    ctx.drawImage(img, rect.x, rect.y, rect.width, rect.height);

    // The frame ahead, at the fraction of the way there. Without it the
    // sequence advances one whole frame at a time, which at 8vh per frame is a
    // visible step no amount of easing removes — the easing only moves
    // smoothly between the same discrete images.
    const ahead = blend.next === null ? null : frames.get(blend.next);
    if (!ahead) return;

    ctx.globalAlpha = blend.mix;
    ctx.drawImage(ahead, rect.x, rect.y, rect.width, rect.height);
    // Reset rather than save/restore: this runs on every animation frame, and
    // the context is otherwise left composing the next paint at partial alpha.
    ctx.globalAlpha = 1;
  };
...
/**
 * A snap anchor per beat, inside the runway, at that beat's scroll position.
 *
 * `top` is the exact inverse of runwayProgress: progress p sits p x (runway
 * height - viewport height) down the runway. Written as CSS rather than
 * computed in JS so it stays correct through a resize with nothing listening
 * for one — the same reason the runway's own height is a vh value.
 *
 * Aligned positions, not authored ones. An anchor that lands mid-frame puts the
 * reader somewhere the picture then has to correct itself, which is the one
 * thing an anchor exists to avoid.
 *
 * No runway means no anchors: the offsets are fractions of its height, and
 * there is nothing to take a fraction of. Scrubbing still works against the
 * document; snapping is the part that cannot.
 *
 * @returns {{ remove: () => void }}
 */
function placeAnchors(container, sections, env) {
  const runway = container.closest?.(`[${RUNWAY_ATTR}]`);
  if (!runway || sections.length === 0) return { remove() {} };

  const placed = [];
  for (const beat of sections) {
    const node = env.document.createElement("div");
    node.className = "st-anchor";
    node.style.top = `calc((100% - 100vh) * ${beat.at})`;
  ...
  return {
    remove() {
      for (const node of placed) runway.removeChild?.(node);
      placed.length = 0;
    },
  };
}
 ...
 */
function paintOverlays({ overlays, story, sequence, exact, eased, focus, vw, vh, ready }) {
  const rect = visibleRect(vw, vh, sequence, computeScale(vw, vh, sequence));
  const sections = story?.sections ?? [];

  overlays.beats.forEach(({ beat, root, scrim }, i) => {
    const opacity = fadeOpacity(sections, i, eased, focus);
