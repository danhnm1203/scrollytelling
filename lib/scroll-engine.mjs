...
export function mount(container, options) {
  ...
  const onResize = () => {
    ...
    env.requestAnimationFrame(() => {
      env.requestAnimationFrame(() => {
        const range = Math.max(1, env.document.body.scrollHeight - env.innerHeight);
        env.scrollTo({ top: target * range });
      });
    });
  };
...
function startScrubbing({ container, sequence, story, framePath, onState, debug, env, log, position }) {
  ...
  const overlays = buildOverlays(container, story, env);
  ...
  const draw = (now) => {
    ...
    paintOverlays({ overlays, story, sequence, exact, eased, vw, vh });
  };
 ...
 */
function buildOverlays(container, story, env) {
 ...
 */
function paintOverlays({ overlays, story, sequence, exact, eased, vw, vh }) {
  ...
  overlays.progressFill.style.width = `${Math.round(eased * 100)}%`;
  overlays.hint.style.opacity = eased > HINT_FADES_AT ? "0" : "1";
}
