...
export function mount(container, options) {
  ...
  if (!sequence) {
    ...
    const noop = () => {};
    live.set(container, noop);
    return noop;
  }

  // Rule 1. Adopt before creating.
  const poster = adoptPoster(container, sequence, framePath, env);
  ...
  const run = startScrubbing({ container, poster, sequence, framePath, onState, debug, env, log });
  const dispose = disposer(container, run);
  live.set(container, dispose);
  return dispose;
}
 ...
 */
function startScrubbing({ container, sequence, framePath, onState, debug, env, log }) {
  ...
  const arrived = (index, bitmap) => {
    ...
    if (next) {
      if (next.phase === "ready" && failed.size > 0) warnFailed(failed, log);
      onState(next.phase === "ready" ? { ...next, sequenceId: sequence.id, progress: eased } : next);
    }
    schedule();
  };
  ...
  if (worker) {
    ...
    worker.onerror = (event) => {
      ...
      const url = event?.filename || String(workerUrl(env));
  ...
  const draw = (now) => {
    ...
    const target = scrollProgress(env.scrollY, env.document.body.scrollHeight, vh);
 ...
 * what lets a bundler find the worker and emit it as its own chunk. A plain
...
function makeWorker(env) {
  ...
  try {
    return new env.Worker(workerUrl(), { type: "module" });
  } catch {
    return null;
  }
}
