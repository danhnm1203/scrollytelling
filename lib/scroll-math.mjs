...
export const SCROLL_MATH_VERSION = "1.3.0";
...
/**
 * How much of the gap between two beats is spent crossfading, the rest held.
 *
 * A crossfade that runs the whole gap means each beat is at full opacity for a
 * single instant and dimmed everywhere else — two beats at 0.35 and 0.65 for
 * most of the scroll, which reads as permanently washed out rather than as a
 * transition. Compressing the fade into the middle of the gap buys back a
 * stretch at each end where exactly one beat is fully lit.
 */
const HANDOFF = 0.45;

/**
 * Ease a crossfade so it is decisive in the middle and gentle at the edges.
 *
 * Both beats sit near 0.5 where the curve crosses, and no curve summing to 1
 * can avoid that. What it can do is pass through quickly: smoothstep across a
 * compressed window leaves about a fifth of the gap ambiguous where a linear
 * ramp across the whole gap leaves seven tenths of it.
 *
 * Smoothstep rather than a steeper power because its derivative is zero at both
 * ends, so the hold meets the fade with no visible corner. A hard edge into a
 * hold band would trade the wash for a flicker.
 */
function handoff(t) {
  const hold = (1 - HANDOFF) / 2;
  if (t <= hold) return 0;
  if (t >= 1 - hold) return 1;

  const u = (t - hold) / HANDOFF;
  return u * u * (3 - 2 * u);
}
 ...
 *
 * `at` is authored against what the footage is doing, which is the right way to
 * write it and has nothing to do with where the frame boundaries fall. So a
 * beat lands mid-frame, and mid-frame is where two frames are composited — the
 * one place a reader is most likely to stop and look is the one place the
 * picture is softest. `settlePosition` corrects it, but only after they have
 * stopped, and on a five-beat story that correction is nearly half a frame at
 * the last beat: a visible shift under a reader who has already come to rest.
 *
 * Moving the beat removes the correction instead of animating it. The move is
 * at most half a frame — under 1% of the sequence at 59 frames — which is far
 * inside the stretch each beat holds at full opacity, so nothing about which
 * copy is on screen changes.
 *
 * Two guarantees this has to keep. The ends stay exactly at the ends, because
 * they are already whole frames and nudging them would leave the very top or
 * bottom of the scroll outside every beat's window. And no two beats may land
 * on the same position: that divides by a zero-width fade window and makes the
 * second beat unreachable. A beat that would collide keeps its authored `at`,
 * which is a worse frame and a working page.
 *
 * @param {readonly {at: number}[]} sections
 * @param {number} totalFrames
 */
export function alignBeats(sections, totalFrames) {
  const last = totalFrames - 1;
  if (!(last > 0) || !sections) return sections;

  const out = [];
  let previous = -Infinity;

  for (const [index, beat] of sections.entries()) {
    const at = clamp(beat.at, 0, 1);
    // The ends are already frames, and are load-bearing as ends.
    const aligned = at <= 0 || at >= 1 ? at : Math.round(at * last) / last;

    // Accept the move only where it cannot disturb its neighbours: past the
    // beat before it, and short of where the beat after it was written. Half a
    // frame is enough to jump a beat on a short sequence with close beats, and
    // an out-of-order list divides by a negative window.
    const next = sections[index + 1];
    const ceiling = next ? clamp(next.at, 0, 1) : Infinity;
    const kept = aligned > previous && aligned < ceiling ? aligned : at;

    out.push(kept === beat.at ? beat : { ...beat, at: kept });
    previous = kept;
  }
...
/**
 * The beat a settled page should resolve onto: whichever is already winning.
 *
 * Read off the same crossfade the page is rendering rather than from the
 * nearest `at`, so the copy resolves onto the beat that is already the more
 * legible of the two. Ties go to the earlier beat — arbitrary, but it has to be
 * fixed, or a position landing exactly between two beats flickers.
 */
function dominantBeat(sections, progress) {
  let best = 0;
  let bestOpacity = -1;
  for (let i = 0; i < sections.length; i++) {
    const opacity = crossfade(sections, i, progress);
 ...
 *
 * `focus` is how far the page has settled: 0 while the scroll is live, 1 once
 * it has stopped and the copy has resolved. A crossfade is the right thing
 * while a reader is moving through the story and the wrong thing the moment
 * they stop — two headings at part opacity is a state to pass through, not one
 * to leave on screen. At `focus` 1 exactly one beat is lit.
 *
 * The pair still sums to 1 at every value of `focus` in between, because both
 * the crossfade and what it resolves to sum to 1 and this interpolates between
 * them. No scroll position is ever left with no copy on it, mid-resolve
 * included.
 *
 * @param {readonly {at: number}[]} sections
 * @param {number} index
 * @param {number} progress
 * @param {number} [focus] 0 = crossfade, 1 = one beat only
 */
export function fadeOpacity(sections, index, progress, focus = 0) {
  const fade = crossfade(sections, index, progress);
  if (!(focus > 0) || sections.length < 2) return fade;

  const resolved = index === dominantBeat(sections, progress) ? 1 : 0;
  return fade + (resolved - fade) * clamp(focus, 0, 1);
}

/**
 * How visible a beat is from the crossfade alone, before any settling.
 *
 ...
 *
 * The window still reaches the neighbour rather than the midpoint, so the two
 * opacities still sum to 1 and no scroll position is left with no copy on it.
 * What `handoff` changes is where inside that window the change happens.
 */
function crossfade(sections, index, progress) {
  ...
  if (p <= at) {
    ...
    return handoff(clamp((p - prev.at) / Math.max(1e-6, at - prev.at), 0, 1));
  }

  if (!next) return 1;
  if (p >= next.at) return 0;
  // The complement of the same shaped `t` the next beat will compute, which is
  // what keeps the pair summing to 1 through the handoff.
  return 1 - handoff(clamp((p - at) / Math.max(1e-6, next.at - at), 0, 1));
}
