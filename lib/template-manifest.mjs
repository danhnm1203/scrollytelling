 ...
 *
 * One data structure, three consumers: `scripts/scaffold.mjs` places files by
 * it, `scripts/frames.mjs` resolves its output paths by it, and
 * `lib/cli-args.mjs` names the templates from it. That is the same shape
 * `installableFiles()` already has, for the same reason — three readers of one
 * list cannot disagree about what a template is.
 *
 * Pure data, no filesystem. `cli-args` documents itself as free of fs, exit and
 * console, and it imports this to list the templates.
 *
