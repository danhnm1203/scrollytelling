...
import { cardFields } from "./social-card.mjs";
 ...
 */
function replaceHead(html, story, options) {
  let out = html;
  ...
  return replaceCard(out, story, options);
}

/**
 * One `<meta>` tag, found by key.
 *
 * Matches on `property` or `name` because Open Graph uses one and Twitter the
 * other. Order-independent: matching `property="…" content="…"` in that order
 * looks fine until someone reformats their head and `content` ends up first,
 * at which point the tag silently stops being rewritten — indistinguishable
 * from one the author deleted on purpose, which is the single distinction this
 * file is built on.
 *
 * Exported because tools/demo-card.mjs checks the tags this writes. A guard
 * with its own copy of this pattern cannot catch a mistake in the pattern —
 * both would agree while the page was wrong — so it shares this one rather
 * than keeping an independence it never actually had.
 *
 * @param {string} html
 * @param {string} key
 * @returns {string | null} the tag's markup, or null when the page lacks it
 */
export function findMeta(html, key) {
  const tag = new RegExp(String.raw`<meta\b[^>]*\b(?:property|name)="${key}"[^>]*>`).exec(html);
  return tag ? tag[0] : null;
}

/** A tag's content attribute, or null when the page has no such tag. */
export function readMeta(html, key) {
  const tag = findMeta(html, key);
  if (tag === null) return null;
  const content = /content="([^"]*)"/.exec(tag);
  return content ? content[1] : null;
}

function fillMeta(html, key, value) {
  const tag = findMeta(html, key);
  if (tag === null) return html;

  // Function replacements, both times. A string replacement interprets `$&`
  // and `$1`, and the value here is the author's copy — a story title
  // containing `$&` would otherwise be replaced by the matched text rather
  // than written out.
  const filled = tag.replace(/content="[^"]*"/, () => `content="${escapeHtml(value)}"`);
  return html.replace(tag, () => filled);
}

/**
 * The link preview, brought into line with the story and the recorded origin.
 *
 * A crawler building a preview never runs the page — it reads the served markup
 * and stops. So whatever the head says at build time is the entire preview, and
 * a page whose head says nothing is a bare link however good the page is.
 *
 * Two rules differ here, on purpose:
 *
 * A value the story does not set is LEFT ALONE, the same as `<title>`: no
 * opinion means keep whatever is there.
 *
 * A value that depends on the site url is CLEARED when there is no site url.
 * Leaving the previous run's absolute url in place would be worse than empty —
 * it would point a crawler at wherever the page used to live. And a relative
 * `og:image` is the specific failure this exists to prevent: it looks correct
 * in the markup and resolves against the crawler's own base, silently fetching
 * something else or nothing.
 */
function replaceCard(html, story, options) {
  // What the card CONTAINS comes from lib/social-card.mjs, which the three
  // framework templates also read during their own builds. This function is
  // only the zero-build template's way of getting it into a head — the
  // mechanism, not the answer. Four mechanisms is right; four answers would
  // drift, and the drift is invisible because every page still renders.
  const card = cardFields({ story, siteUrl: options?.siteUrl, cardPath: options?.cardPath });
  let out = html;

  if (card.title !== null) {
    out = fillMeta(out, "og:title", card.title);
    out = fillMeta(out, "twitter:title", card.title);
  }
  if (card.description !== null) {
    out = fillMeta(out, "og:description", card.description);
    out = fillMeta(out, "twitter:description", card.description);
  }

  out = fillMeta(out, "og:type", card.type);
  out = fillMeta(out, "twitter:card", card.twitterCard);
  out = fillMeta(out, "og:url", card.url ?? "");
  out = fillMeta(out, "og:image", card.image ?? "");
  out = fillMeta(out, "twitter:image", card.image ?? "");
 ...
 * @param {string} html
 * @param {object} story
 * @param {{ siteUrl?: string, cardPath?: string }} [options]
 */
export function replaceOutline(html, story, options) {
  html = replaceHead(html, story, options);
