...
function fillMeta(html, key, value) {
  // Two steps rather than one pattern, so attribute ORDER does not matter.
  // Matching `property="…" content="…"` in that order looks fine until someone
  // reformats their head and `content` ends up first: the tag stops being
  // rewritten, and a tag that is silently never updated is indistinguishable
  // from one the author deleted on purpose. Find the tag by its key, then
  // replace whatever content attribute it has.
  const tag = new RegExp(String.raw`<meta\b[^>]*\b(?:property|name)="${key}"[^>]*>`);
