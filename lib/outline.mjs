...
/**
 * Fills one `<meta>` tag, if the page has it.
 *
 * Matches on `property` or `name` because Open Graph uses one and Twitter the
 * other, and a page is free to have either. Absent means the author removed it,
 * which is an instruction, not an oversight.
 */
/**
...
function replaceCard(html, story, options) {
  ...
  if (story?.title) {
    out = fillMeta(out, "og:title", story.title);
    out = fillMeta(out, "twitter:title", story.title);
  }
  if (story?.description) {
    out = fillMeta(out, "og:description", story.description);
    out = fillMeta(out, "twitter:description", story.description);
  }

  const card = siteUrl && cardPath ? new URL(cardPath, siteUrl).href : "";
  out = fillMeta(out, "og:url", siteUrl ?? "");
  out = fillMeta(out, "og:image", card);
  out = fillMeta(out, "twitter:image", card);
